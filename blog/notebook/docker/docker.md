...
// 2、Image打包
docker build \
	-t xuxueli/xxl-job-admin:{指定版本} ./xxl-job-admin \
...
// 2、指定架构 pull Image（AMD架构）
docker pull eclipse-temurin:17-jre --platform linux/amd64
...
// 2、登陆Hub：
docker login (或 docker login docker.io )

// 3、tag镜像：
docker tag <imageID> <namespace>/<image name>:<version tag eg latest>

// 4、push镜像：
docker push <namespace>/<image name>:<version tag eg latest>
```
