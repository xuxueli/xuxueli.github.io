...
- 官方插件管理器（Package Control）安装: 
  - 输入快捷键 `Cmd+Shift+P` 唤起命令面板；
  - 输入 `Install Package Control` 并回车；等待安装完成，通常会弹出一个对话框提示成功。
  - 输入 `Package Control: Install Package` 并回车

- 插件安装：
  - 输入快捷键 `Cmd+Shift+P` 打开命令面板；
  - 输入 `Package Control: Install Package` 并回车，稍等片刻，待插件列表加载完成后，可查询并点击下载安装插件。

- 插件推荐：
  - Pretty JSON:: 格式化JSON代码
  ```
  # 格式化/美化JSON
  1、选中需要格式化的JSON文本（不选中则默认处理整个文件）
  2、命令面板搜索 "Pretty JSON: Format JSON" 并执行
  3、默认快捷键：Ctrl+Alt+J（Windows/Linux）/Cmd+Alt+J（macOS）
  
  # 压缩JSON
  1、命令面板搜索 “Pretty JSON: Minify JSON” 并执行；
  
  # 其他实用功能
  1、JSON转XML‌：执行 “Pretty JSON: json2xml” 即可完成格式转换，自动切换XML语法高亮。
  2、键排序‌：如需对JSON键进行字母排序，可执行 “Pretty JSON: Format JSON with Sort” 。
  3、‌符号跳转‌：执行 “Pretty JSON: Goto Symbol” 可快速定位大型JSON文件中的指定键名。
  ```
  - MarkdownEditing：增强Markdown编辑功能，如命令面板执行 “MarkdownEditing: New Image” 快捷插入图片；
  - Markdown Preview：实时预览Markdown文件，如命令面板执行 “Preview in Browser” 在默认浏览器打开实时预览；
  - LiveReload: 实时刷新网页，如命令面板执行 “LiveReload: Enable/disable plugins” 并选择 “Enable：Simple” 开启实时刷新功能；
