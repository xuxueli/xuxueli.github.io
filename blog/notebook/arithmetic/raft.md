...
![](https://www.xuxueli.com/blog/static/images/default/img_255.jpg)
<center>(单机环境)</center>
...
![](https://www.xuxueli.com/blog/static/images/default/img_250.png)
<center>(分布式环境)</center>
...
![](https://www.xuxueli.com/blog/static/images/default/img_247.png)
<center>(节点状态图)</center>

初始化状态时，三个节点都是Follower状态，并且term为0，如下图所示：

![](https://www.xuxueli.com/blog/static/images/default/img_260.png)
<center>(初始化)</center>
...
![](https://www.xuxueli.com/blog/static/images/default/img_257.png)
...
![](https://www.xuxueli.com/blog/static/images/default/img_258.png)
...
![](https://www.xuxueli.com/blog/static/images/default/img_239.png)

为了提交这条日志，Leader会将这条日志通过心跳消息复制给其他的Follower节点：

![](https://www.xuxueli.com/blog/static/images/default/img_248.png)
<center>(日志复制)</center>

一旦有大多数节点成功写入这条日志，那么Leader节点的这条日志状态就会更新为committed状态，并且值更新为5：

![](https://www.xuxueli.com/blog/static/images/default/img_252.png)

Leader节点然后通知其他Follower节点，其他节点也会将值更新为5。如下图所示，这个时候集群的状态是完全一致的，这个过程就叫做 **日志复制（Log Replication）** ：

![](https://www.xuxueli.com/blog/static/images/default/img_259.png)
...
![](https://www.xuxueli.com/blog/static/images/default/img_245.jpg)
<center>(选举超时)</center>
...
![](https://www.xuxueli.com/blog/static/images/default/img_253.png)
<center>(心跳超时)</center>
...
![](https://www.xuxueli.com/blog/static/images/default/img_241.png)
<center>(re-election)</center>

如果节点A能得得到节点C的投票，加上自己的投票，就有大多数选票。那么节点A将成为新的Leader节点，并且Term即任期的值加1更新到2：

![](https://www.xuxueli.com/blog/static/images/default/img_244.png)
...
![](https://www.xuxueli.com/blog/static/images/default/img_256.png)
<center>(2个Candidate节点)</center>

这种情况下，就会触发一次新的选举，节点A和节点B又等待一个随机的选举超时时间，直到一方胜出：

![](https://www.xuxueli.com/blog/static/images/default/img_249.png)

我们假设节点A能得到大多数投票，那么接下来节点A就会成为新的Leader节点，并且任期term加1：

![](https://www.xuxueli.com/blog/static/images/default/img_246.png)

## 网络分区

在发生网络分区的时候，Raft一样能保持一致性。如下图所示，假设我们的集群由5个节点组成，且节点B是Leader节点：

![](https://www.xuxueli.com/blog/static/images/default/img_240.png)
<center>(5个节点的集群)</center>

我们假设发生了网络分区：节点A和B在一个网络分区，节点C、D和E在另一个网络分区，如下图所示，且节点B和节点C分别是两个网络分区中的Leader节点：

![](https://www.xuxueli.com/blog/static/images/default/img_242.png)
...
![](https://www.xuxueli.com/blog/static/images/default/img_243.png)
<center>(操作1)</center>

另一个客户端准备执行SET 8的操作，由于可以被同一个分区下总计三个节点（节点C、D和E）感知到，3个节点已经符合大多数节点的条件。所以，这个值的状态就是committed：

![](https://www.xuxueli.com/blog/static/images/default/img_251.png)
<center>(操作2)</center>
...
![](https://www.xuxueli.com/blog/static/images/default/img_254.png)
